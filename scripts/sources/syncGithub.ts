...
async function main() {
  ...
  // Release lookups are calls to GitHub, not Neon — batching those
  // wouldn't reduce DB round trips, so this loop stays as-is. It's the
  // per-repo Neon INSERT that got collapsed below.
  ...
  if (repos.length > 0) {
    ...
    await sql`
      insert into github_cache (
        ...
        latest_release, updated_at, fetched_at
      )
      select
        r.repo, r.language, r.homepage,
        array(select jsonb_array_elements_text(r.topics)) as topics,
        r.stars, r.forks, r.open_issues, r.latest_release, r.updated_at, now()
