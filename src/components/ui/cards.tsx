    ...
    <div onClick={()=>handleNavigation(id)} 
     className="group flex-shrink-0 w-[16rem] h-40 md:w-[32rem] md:h-80 rounded-2xl overflow-hidden shadow-[inset_0_1px_2px_rgba(255,255,255,0.05),inset_0_-1px_2px_rgba(0,0,0,0.4),0_4px_12px_rgba(0,0,0,0.4)] border border-gray-600/30 transition-all duration-300 hover:shadow-[inset_0_1px_2px_rgba(255,255,255,0.1),inset_0_-1px_2px_rgba(0,0,0,0.5),0_6px_20px_rgba(0,0,0,0.5)] bg-slate-900 cursor-pointer">      
      ...
      <div className={`h-full w-full flex flex-col justify-between relative`}>
            ...
            {cover ? (
              ...
              <div className="w-full h-full rounded-lg bg-slate-800/40 flex items-center justify-center border border-slate-700/40 backdrop-blur-sm">
                <div className="text-center">
                  <h2 className="text-[11px] md:text-3xl font-bold text-white mb-1 md:mb-4 tracking-wide">
                    {title}
                  </h2>
                  <div className="text-slate-400 text-[9px] md:text-sm mb-1 md:mb-4">Replace with cover</div>
                  <Image size={12} className="mx-auto text-slate-500 md:hidden" />
                  <Image size={20} className="mx-auto text-slate-500 hidden md:block" />
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex gap-2 md:gap-4 items-center p-2 md:p-4 pt-2 md:pt-3 bg-slate-900 relative z-10">
          <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-slate-700/50 border border-slate-600/50 flex items-center justify-center flex-shrink-0">
            <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-slate-400"></div>
          </div>
          <div>
            <h3 className="font-medium text-white text-[10px] md:text-base tracking-wide">{title}</h3>
            <p className="text-slate-300 text-[8px] md:text-sm mt-0.5 md:mt-1">{subtitle}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="h-full w-full flex flex-col justify-between relative">
          ...
          <div className="w-full h-full flex items-center justify-center relative z-10">
            {cover ? (
              ...
              <div className="w-full h-full rounded-lg bg-slate-800/40 flex flex-col items-center justify-center border border-slate-700/40 backdrop-blur-sm gap-2 md:gap-3 px-6">
                <div
                  className="w-8 h-8 md:w-12 md:h-12 rounded-xl flex items-center justify-center text-xs md:text-sm font-bold border"
                  style={{
                ...
                <h2 className="text-[11px] md:text-2xl font-bold text-white text-center tracking-wide leading-snug">
                  {role}
                </h2>

                <span
                  className="text-[10px] md:text-[11px] font-mono px-3 py-1 rounded-full border"
                  style={{
                    color: accentColor,
        ...
        <div className="flex gap-2 md:gap-4 items-center p-2 md:p-4 pt-2 md:pt-3 bg-slate-900 relative z-10">
          <div
            className="w-6 h-6 md:w-8 md:h-8 rounded-full border flex items-center justify-center flex-shrink-0"
            style={{
              background: `${accentColor}15`,
              borderColor: `${accentColor}35`,
            }}
          >
            <Briefcase size={10} style={{ color: accentColor }} className="md:hidden" />
            <Briefcase size={14} style={{ color: accentColor }} className="hidden md:block" />
          </div>

          <div className="flex-1 min-w-0">
            <h3 className="font-medium text-white text-[10px] md:text-base tracking-wide truncate">
              {role}
            </h3>
            <p className="text-slate-300 text-[8px] md:text-sm mt-0.5 md:mt-1">{company}</p>
          </div>
          ...
          <div className="flex gap-1.5 flex-shrink-0">
            {tags.slice(0, 2).map((tag) => (
              <span
                key={tag}
                className="text-[9px] md:text-[10px] px-1.5 md:px-2 py-0.5 rounded-full border text-white/40"
                style={{
...
function SkillsContent({ item }: { item: ExpertiseEntry }) {
  return (
    <div className="w-full h-full flex flex-col justify-center gap-2 md:gap-3 px-1">
      {item.skillCategories?.map((cat) => (
        <div key={cat.label}>
          <p
            className="text-[10px] md:text-[11px] font-mono mb-1 md:mb-1.5"
          ...
          <div className="flex flex-wrap gap-1 md:gap-1.5">
            {cat.skills.map((skill) => (
              <span
                key={skill}
                className="text-[9px] md:text-[11px] px-1.5 md:px-2.5 py-0.5 rounded-full border text-white/50 hover:text-white/80 transition-colors"
                style={{
...
function ToolsContent({ item }: { item: ExpertiseEntry }) {
  return (
    <div className="w-full h-full grid grid-cols-3 gap-1.5 md:gap-2 content-center">
      {item.tools?.map((tool) => (
        <div
          key={tool.name}
          className="flex flex-col items-center gap-1 md:gap-1.5 py-1.5 md:py-2 px-1 rounded-xl border transition-colors group/tool"
          style={{
        ...
        >
          ...
          <span className="text-[8px] md:text-[10px] text-white/40 group-hover/tool:text-white/65 transition-colors text-center leading-tight">
            {tool.name}
          </span>
        </div>
      ))}
...
function CVContent({ item }: { item: ExpertiseEntry }) {
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between">
      <div>
        <p className="text-white text-[9px] md:text-[15px] font-semibold leading-tight">{cv.name}</p>
        <p className="text-white/40 text-[7px] md:text-[13px] mt-0.5 md:mt-1">{cv.title}</p>
        <div className="flex items-center gap-2 mt-1 md:mt-2">
          <span className="text-[7px] md:text-[11px] font-mono text-white/25">Updated {cv.lastUpdated}</span>
          <span className="text-[7px] md:text-[11px] font-mono text-white/25">PDF · {cv.fileSize}</span>
        </div>

        <div
          className="mt-2 md:mt-3 p-2 md:p-3 rounded-xl border"
          style={{
            background: "rgba(255,255,255,0.03)",
            borderColor: "rgba(255,255,255,0.07)",
          }}
        >
          <div className="h-[4px] md:h-[5px] w-3/5 rounded-full bg-white/10 mb-1.5 md:mb-2" />
          <div className="h-[3px] md:h-[4px] w-11/12 rounded-full bg-white/[0.06] mb-1 md:mb-1.5" />
          <div className="h-[3px] md:h-[4px] w-4/5 rounded-full bg-white/[0.06] mb-1 md:mb-1.5" />
          <div className="h-[3px] md:h-[4px] w-2/3 rounded-full bg-white/[0.06]" />
        </div>
      </div>
      ...
      <a
        ...
        className="mt-2 md:mt-3 flex items-center justify-center gap-2 w-full py-1.5 md:py-2.5 rounded-xl text-[8px] md:text-[13px] font-semibold transition-all duration-200 hover:opacity-90 active:scale-[0.98] border"
        style={{
  ...
  return (
    ...
    >
      <div className="h-full w-full flex flex-col justify-between relative">
        ...
        <div className={`flex-1 p-2 md:p-4 bg-gradient-to-br ${bgGradient} relative overflow-hidden`}>
          <div
            className={`absolute inset-0 bg-gradient-to-br ${hoverGradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
          />
        ...
        <div className="flex gap-2 md:gap-4 items-center p-2 md:p-4 pt-2 md:pt-3 bg-slate-900 relative z-10">
          <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-slate-700/50 border border-slate-600/50 flex items-center justify-center text-sm md:text-base flex-shrink-0">
            {bottomIcon[item.type]}
          </div>
          <div>
            <h3 className="font-medium text-white text-[10px] md:text-base tracking-wide">{label.title}</h3>
            <p className="text-slate-300 text-[8px] md:text-sm mt-0.5 md:mt-1">{label.subtitle}</p>
          </div>
        </div>
