...
//     singleProjects.find((p) => p.id === id);
 ...
 /**
 * ═══════════════════════════════════════════════════════════════
 * SINGLE PROJECT DATA — AUTO REGISTRY
 * ═══════════════════════════════════════════════════════════════
 *
 * All entries in ./entries/*.ts are automatically imported.
 */
...
export interface ProjectFeature {
  ...
  // ─────────────────────────────────────────────
  // Auto Import All Project Entries
  // ─────────────────────────────────────────────
  const modules = import.meta.glob("./entries/*.ts", {
	eager: true,
	import: "default",
  }) as Record<string, SingleProject>;
  
  // Main export
  export const singleProjects: SingleProject[] = Object.values(modules);
  
  // Sorted version (newest first)
  ...
  export const getProjectsByType = (type: string): SingleProject[] =>
	singleProjects.filter((p) => p.id.toLowerCase().startsWith(type.toLowerCase()));
  
  export const getAllProjectTypes = (): string[] => {
	const types = singleProjects.map((p) => p.id.split("-")[0]);
